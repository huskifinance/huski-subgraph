...
export class LendDayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save LendDayData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save LendDayData entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("LendDayData", id.toString(), this);
    }
  }

  static load(id: string): LendDayData | null {
    return changetype<LendDayData | null>(store.get("LendDayData", id));
  }
  ...
  get dailyValue(): BigInt | null {
    let value = this.get("dailyValue");
    if (!value || value.kind == ValueKind.NULL) {
      return null;
    } else {
      return value.toBigInt();
  ...
  set dailyValue(value: BigInt | null) {
    if (!value) {
      this.unset("dailyValue");
    } else {
      this.set("dailyValue", Value.fromBigInt(<BigInt>value));
    }
  }

  get dailyBaseAmount(): BigInt | null {
    let value = this.get("dailyBaseAmount");
    if (!value || value.kind == ValueKind.NULL) {
      return null;
    } else {
      return value.toBigInt();
  ...
  set dailyBaseAmount(value: BigInt | null) {
    if (!value) {
      this.unset("dailyBaseAmount");
    } else {
      this.set("dailyBaseAmount", Value.fromBigInt(<BigInt>value));
    }
  }
}
